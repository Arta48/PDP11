...
*   **Student Verification & Academic Integrity:** Supports secure authentication through the university Moodle portal (`edu.vsu.ru`). The emulator automatically detects whether the logged-in user is a student or a teacher, dynamically applying correct academic integrity rules. Saved `.pdp` files are bound to the student ID via hardware-backed cryptographic signatures (HMAC-SHA256) to ensure authorship and prevent unauthorized file sharing.
