...
rm -rf build-macos dmg_root PDP11.dmg
mkdir -p build-macos
...
if command -v macdeployqt &> /dev/null; then
    macdeployqt "build-macos/Command System Emulator PDP-11.app" 2>&1 | grep -v -E "Cannot resolve rpath|using QList|using QSet" || true
fi
...
if command -v create-dmg &> /dev/null; then
    mkdir -p dmg_root
    cp -R "build-macos/Command System Emulator PDP-11.app" dmg_root/
    if [ -f Docs/PDP11.pdf ]; then
        cp Docs/PDP11.pdf dmg_root/
    fi
    if [ -f "Docs/PDP11 RU.pdf" ]; then
        cp "Docs/PDP11 RU.pdf" dmg_root/
    fi
